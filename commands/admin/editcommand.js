exports.cmd = new Oxyl.Command("editcommand", async message => {
	...
	if(message.args[1] === "reset") {
		await framework.resetCommand(cmd.name, message.channel.guild);
		return `Reset command \`${cmd.name}\``;
	} else if(message.args[1] === "toggle") {
		let res = await framework.editCommand(cmd.name, "toggle", message.channel.guild);
		...
		await framework.editCommand(cmd.name, roles, message.channel.guild);

...
async function handleRole(message, args) {
	if(!args || !args[0] || args[0].toLowerCase() === "help") {
		...
		let available = await framework.getRoles(message.guild, "me");
		available = available
			.filter(data => message.guild.roles.has(data.ROLE))
			.map(data => message.guild.roles.get(data.ROLE).name);
		...
		let role = parseRole(message.guild, args.splice(1).join(" "));
		if(!role) return "Invalid role! Role not found.";

		let exists = await framework.getRole(message.guild, "me", role);
		if(exists) return `\`${role.name}\` is already an available role`;

		framework.addRole(message.guild, "me", role);
		...
		framework.deleteRole(message.guild, "me", role);
		return `Removed role \`${role.name}\` from available roles`;
	} else {
		let role = parseRole(message.guild, args.join(" "));

...
module.exports = {
	async run({ args: [player], t }) {
		const [data] = await request().setEndpoint("getplayer").data(player);
		if(!data) return t("commands.paladins.player.invalidPlayer");

		const championRanks = await request().setEndpoint("getchampionranks").data(player);
		...
		ctx.fillText(`Overview: ${data.Name}`, 300, 15);
		...
		ctx.fillText(data.Level, 80, 120);

		ctx.fillStyle = "#797D7F";
		ctx.fillRect(168, 35, 250, 25);

		ctx.fillStyle = "#90CE49";
		ctx.fillRect(170.5, 37.5, 245 * Math.min(data.RankedConquest.Points, 100) / 100, 20);

		ctx.fillStyle = "white";
		ctx.font = "bold 16px Roboto";
		ctx.fillText(`${data.RankedConquest.Points} TP`, 293, 53);
		...
		do {
			...
			fontSize--;
		} while(ctx.measureText(ranks[data.RankedConquest.Tier]).width > 155);
		ctx.fillText(ranks[data.RankedConquest.Tier], 260, 100);

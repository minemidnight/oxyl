	...
	if(req.query.code) {
		try {
			var { body } = await superagent
				...
				.type("form")
				.send({
					client_id: app.config.website.botID, // eslint-disable-line camelcase
					client_secret: app.config.website.secret, // eslint-disable-line camelcase
					code: req.query.code,
					grant_type: "authorization_code", // eslint-disable-line camelcase
					redirect_uri: `${app.config.website.baseURL}/callback` // eslint-disable-line camelcase
				});
		} catch(err) {
			res.redirect(`https://discordapp.com/oauth2/authorize?response_type=code&redirect_uri=` +
					`${app.config.website.baseURL}/callback&scope=identify+guilds&client_id=${app.config.website.botID}`);
			return;
		}
		...
		res.status(200).send(`<script>` +
				...
				`window.location.href = "${app.config.website.baseURL}/accounts"` +
				`</script>`);
